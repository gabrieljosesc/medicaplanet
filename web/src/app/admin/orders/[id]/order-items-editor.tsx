...
import {
  saveOrderItemsAction,
  searchOrderProductsAction,
  type AdminProductSuggestion,
...
export function OrderItemsEditor({
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true);
      try {
        setResults(await searchOrderProductsAction(query));
      } catch {
        /* ignore */
      } finally {
        setSearching(false);
  ...
  function addProduct(s: AdminProductSuggestion) {
    setItems((prev) => [
      ...prev,
  ...
  function save() {
    ...
    startTransition(async () => {
      const res = await saveOrderItemsAction({
        orderId,
        items: items.map(({ product_id, title, quantity, unit_price }) => ({
          product_id,
          title,
          quantity,
          unit_price,
      ...
      setMsg(res.ok ? { ok: true, text: "Order saved." } : { ok: false, text: res.message });
    });
  }

  const inputCls = "rounded-md border border-zinc-300 px-2 py-1 text-sm";
    ...
    <section className="mt-4 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="text-xs text-zinc-400">
          Adjust quantity, price, or discount; add or remove products.
        </span>
      </div>

      <div className="mt-3 divide-y divide-zinc-100">
        {items.length === 0 && (
          <p className="py-3 text-sm text-zinc-400">No items — add a product below.</p>
        )}
        {items.map((it) => (
          <div key={it.key} className="flex flex-wrap items-center gap-3 py-2">
            ...
            <label className="flex items-center gap-1 text-xs text-zinc-500">
              ...
              <input
                ...
                value={it.unit_price}
                onChange={(e) =>
                  updateItem(it.key, { unit_price: Math.max(0, Number(e.target.value) || 0) })
                }
                className={`${inputCls} w-24`}
              />
            </label>
            <span className="w-20 text-right text-sm font-medium text-zinc-900">
      ...
      <div className="relative mt-3">
        ...
        {results.length > 0 && (
          <div className="absolute left-0 right-0 z-10 mt-1 max-h-60 overflow-auto rounded-lg border border-zinc-200 bg-white shadow-lg">
            {results.map((r) => (
              <button
                key={r.id}
                type="button"
              ...
              >
                ...
                <span className="flex-shrink-0 text-zinc-500">{money(r.base_price)} +</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div className="mt-4 space-y-1.5 border-t border-zinc-100 pt-3 text-sm">
        ...
        <div className="flex items-center justify-between gap-2 text-zinc-600">
          <span>
            Discount
            {couponCode ? (
              <span className="ml-1 text-xs text-zinc-400">(coupon {couponCode})</span>
            ) : null}
          </span>
      ...
      {msg && (
        <p className={`mt-3 text-sm ${msg.ok ? "text-teal-700" : "text-red-600"}`}>{msg.text}</p>
      )}
      <button
        type="button"
        onClick={save}
        disabled={pending}
        className="mt-3 rounded-full bg-teal-800 px-6 py-2.5 text-sm font-semibold text-white hover:bg-teal-900 disabled:opacity-50"
      >
