...
export default async function AdminUsersPage({ searchParams }: Props) {
  ...
  const { data: profiles } = await svc
    .from("profiles")
    .select("id,email,first_name,last_name,full_name,phone,company,role,created_at")
    .order("created_at", { ascending: false })
    .limit(500);

  const rows = (profiles ?? []).filter((p) => {
    if (!q) return true;
    const haystack = [p.email, p.full_name, p.first_name, p.last_name, p.phone, p.company]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
          ...
          <tbody>
            {rows.map((u) => (
              <tr key={u.id} className="border-b border-zinc-100 hover:bg-zinc-50">
                ...
                <td className="py-2 pr-3 text-zinc-500">{u.phone ?? "—"}</td>
